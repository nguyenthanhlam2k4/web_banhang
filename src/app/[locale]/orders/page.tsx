            ...
            {filteredOrders.map((order) => (
              ...
              >
                <div className="p-6 md:p-8 flex flex-col md:flex-row gap-8">
                  ...
                  {/* Actions */}
                  <div className="flex flex-col items-stretch justify-center gap-3 border-t md:border-t-0 md:border-l border-border/50 pt-6 md:pt-0 md:pl-8 min-w-[160px]">
                    <Link href={`/orders/${order._id}`} className="w-full">
                      <Button variant="ghost" className="w-full rounded-xl h-11 gap-2 font-bold group-hover:bg-primary/10 group-hover:text-primary transition-all border border-transparent group-hover:border-primary/20">
                    ...
                    {order.orderStatus === 'delivered' && (
                      <Link href={`/orders/${order._id}`} className="w-full">
                        <Button variant="outline" className="w-full rounded-xl h-11 gap-2 font-black text-xs border-emerald-500/30 text-emerald-600 hover:bg-emerald-500 hover:text-white transition-all shadow-sm">
                    ...
                    {!order.isPaid && order.orderStatus !== 'cancelled' && order.checkoutUrl && (
                      <Link href={order.checkoutUrl} className="w-full">
                        <Button variant="outline" className="w-full rounded-xl h-11 font-black text-xs uppercase tracking-wider text-primary border-primary/20 hover:bg-primary/5">
                          {t('payNow')}
                        </Button>
                      </Link>
                    )}
